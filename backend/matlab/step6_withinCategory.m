function step6_withinCategory(teamName,whichData)
% step6_withinCategory('bayesianmath','BrainPrediction')
if nargin<2
...
for i=1:length(teams)
    ...
    load(fileName);
    yy(1,i)=Results.ave_accuracySameCat;
    ee(1,i)=std(Results.accuracySameCat)/sqrt(length(Results.accuracySameCat));
    YY(:,i)=Results.accuracySameCat';
end
...
set(gcf,'Name',sprintf('%s',[whichData ', for pairs items from the same category']));

for i=1:length(teams)
    if (i==1)
        text(i-.12,.55,sprintf('%3.3f',yy(i)),'FontName','Helvetica','FontSize',fontSize);
    else
        text(i-.12,yy(i)+.02,sprintf('%3.3f',yy(i)),'FontName','Helvetica','FontSize',fontSize);
    end
end