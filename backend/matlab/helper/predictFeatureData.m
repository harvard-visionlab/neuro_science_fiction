function Results=predictFeatureData(subnum,featureFileName,fmriFileName,prefs)

%% predict feature data from brain data
...
for i=1:length(ratingItemNames)
    if ~(strcmp(ratingItemNames{i},brainItemNames{i}))
        error('items in different order!');
    end
end
...
%% predict feature data for all possible pairs
...
for i=1:numItems-1
    for j=i:numItems
        if (i~=j)
            ...
            actualFeatures=R(testItems,:);
            actualBrainData=D(testItems,:);
            
            sameCat(c)=double(categoryNum(i)==categoryNum(j));
            
            %predictedBrainData=predictBrainDataFromFeatures(trainBrainData
            %,trainItemFeatures,testItemFeatures);
%             [COEFF, SCORE, LATENT, TSQUARED] = princomp(trainBrainData);
%             numPC=30;
%             trainBrainData=SCORE(:,1:numPC);
%             [b] = glmfitmulti(trainBrainData,trainItemFeatures);
%             allB(:,:,c)=b;
%             for item=1:length(testItems)
%                 pcaBrainData=actualBrainData*COEFF;
%                 brainPattern=repmat([1; trainBrainData(item,:)'],[1 size(trainBrainData,2)]);
%                 predictedFeatures(item,:) = sum(b.*brainPattern,1);
%             end
            
            predictedFeatures=predictFeaturesFromBrainData(trainBrainData,trainItemFeatures,actualBrainData);
            
            % compare actual to predicted brain data for test items
            gotItRight(c)=compareActualPredicted(actualFeatures,predictedFeatures,prefs);
