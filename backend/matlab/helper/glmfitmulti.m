function [bb,dev,stats] = glmfitmulti(x,y,distr,varargin)
...
bb = zeros(ncolx,size(y,2),dataClass); bb(perm,:) = b;
...
if nargout > 2
    ...
    if ~isnan(stats.s) % dfe > 0 or estdisp == 'off'
        RI = R\eye(p);
        C = RI * RI';
        if isequal(estdisp,'on'), C = repmat(C,[1 1 size(y,2)]) .* repmat(reshape(stats.s.^2,[1 1 size(y,2)]),[size(C) 1]); end
        se = sqrt(reshape(C(logical(repmat(eye(p,p),[1 1 size(y,2)]))),[p size(y,2)])); 
        
        %se = se(:);   % insure vector even if empty
        stats.covb = zeros(ncolx,ncolx,size(y,2),dataClass);
        stats.covb(perm,perm,:) = C;
        %C = C ./ (se * se');
        xx=repmat(reshape(se,[p 1 size(y,2)]),[1 p 1]);
        yy=repmat(reshape(se',[1 p size(y,2)]),[p 1 1]);
        C = C ./ (xx .* yy);
        stats.se = zeros(ncolx,size(y,2),dataClass); stats.se(perm,:) = se;
        stats.coeffcorr = zeros(ncolx,ncolx,size(y,2),dataClass);
        stats.coeffcorr(perm,perm,:) = C;
        stats.t = NaN(ncolx,size(y,2),dataClass); stats.t(perm,:) = b ./ se;
