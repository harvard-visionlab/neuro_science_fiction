...
# Test Mitchell run-analysis Lambda function (FAST mode)
# testIndividualFeatures=false (~20 seconds)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common.sh"

echo_test "Testing mitchell-run-analysis function (FAST MODE)"
...
# Send test request
echo ""
echo_info "Sending analysis request (this will take ~20 seconds)..."
START=$(date +%s)
